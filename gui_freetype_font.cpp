...
CGUITTFace::CGUITTFace(IrrlichtDevice* device)
: device(device),face(0)
{
	++countClassObjects;
}
...
bool CGUITTFace::load(const irr::io::path& filename)
{
    ...
    face_buffer = new FT_Byte[file->getSize()];
    file->read(face_buffer,file->getSize());
    face_buffer_size = file->getSize();
    file->drop();

    if(FT_New_Memory_Face(library, face_buffer, face_buffer_size, 0, &face))
	{
		return	false;
	}
	return	true;
...
CGUIFreetypeFont::CGUIFreetypeFont(irr::IrrlichtDevice* device)
: device(device),Driver(device->getVideoDriver()), TrueTypeFace(0)
{
	#ifdef _DEBUG
